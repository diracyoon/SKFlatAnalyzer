...
void AnalyzerCore::SetupJECUncertainty(TString source, TString JetType)
{
  ...
  if (GetEra() == "2016preVFP")
    file = analysisdir + "/" + string(GetEra()) + "/JME/RegroupedV2_Summer16_07Aug2017_V11_MC_UncertaintySources_" + string(JetType) + ".txt";
  else if (GetEra() == "2016postVFP")
    file = analysisdir + "/" + string(GetEra()) + "/JME/RegroupedV2_Summer16_07Aug2017_V11_MC_UncertaintySources_" + string(JetType) + ".txt";
  else if (GetEra() == "2017")
    file = analysisdir + "/" + string(GetEra()) + "/JME/RegroupedV2_Fall17_17Nov2017_V32_MC_UncertaintySources_" + string(JetType) + ".txt";
  else if (GetEra() == "2018")
    file = analysisdir + "/" + string(GetEra()) + "/JME/RegroupedV2_Autumn18_V19_MC_UncertaintySources_" + string(JetType) + ".txt";
  else
  {
  ...
  for (unsigned int ilines = 0; ilines < SourceLines.size(); ilines++)
  {
    ...
    for (int i = 0; i < nBins; i++)
    {
      ...
      if ((i % 3) == 0)
      {
        ptbin.push_back(value_from_string);
        if (i == nBins - 3)
          finalbin = true;
      }
      if ((i % 3) == 1)
      {
        unc_up.push_back(value_from_string);
      }
      if ((i % 3) == 2)
      {
        unc_down.push_back(value_from_string);
      }
  ...
  if (JetType == "AK4PFchs")
    AK4CHSJECUncMap[source] = vec_unc;
  if (JetType == "AK4PFPuppi")
    AK4PUPPIJECUncMap[source] = vec_unc;
  if (JetType == "AK8PFchs")
    AK8CHSJECUncMap[source] = vec_unc;
  if (JetType == "AK8PFPuppi")
    AK8PUPPIJECUncMap[source] = vec_unc;
...
float AnalyzerCore::GetJECUncertainty(TString source, TString JetType, float eta, float pt, int sys)
{
  ...
  if (JetType == "AK4PFchs")
  {
    ...
    if (mapit == AK4CHSJECUncMap.end())
      NotFound = true;
  }
  if (JetType == "AK4PFPuppi")
  {
    mapit = AK4PUPPIJECUncMap.find(source);
    if (mapit == AK4PUPPIJECUncMap.end())
      NotFound = true;
  }
  if (JetType == "AK8PFchs")
  {
    mapit = AK8CHSJECUncMap.find(source);
    if (mapit == AK8CHSJECUncMap.end())
      NotFound = true;
  }
  if (JetType == "AK8PFPuppi")
  {
    mapit = AK8PUPPIJECUncMap.find(source);
    if (mapit == AK8PUPPIJECUncMap.end())
      NotFound = true;
  }
  if (NotFound)
  {
    cout << "ERROR, " << source << " not found in JEC Uncertainty MAP for " << JetType << endl;
    return -999.;
  }
  ...
  for (unsigned int i = 0; i < etabins.size() - 1; i++)
  {
    ...
    {
      bin_boundary = float(etabins.at(i));
      break;
    }
  }
  ...
  {
    if (float(pit->first) == float(bin_boundary))
    {
      ptbins = pit->second;
    }
  }
  ...
  {
    if (pt >= ptbins.at(j) && pt < ptbins.at(j + 1))
    {
      ptbin = j;
      break;
    }
  ...
  if (sys > 0)
    mapit_unc = mapit->second.at(1).find(bin_boundary);
  else
    mapit_unc = mapit->second.at(2).find(bin_boundary);
