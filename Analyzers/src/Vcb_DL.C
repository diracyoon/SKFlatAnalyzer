...
void Vcb_DL::initializeAnalyzer()
{
  ...
  // // 1. 에너지 관련 중복 (SetPtEtaPhiM으로 설정하므로 jet_energy는 읽을 필요 없음)
  // fChain->SetBranchStatus("jet_energy", 0);

  // // 2. Parton PDG ID (jet_partonFlavour는 쓰지만, PartonPdgId는 안 씀)
  // fChain->SetBranchStatus("jet_partonPdgId", 0);

  // // 3. Vertex 관련 정보 (Jet 클래스에 저장 공간 없음)
  // fChain->SetBranchStatus("jet_vtxNtracks", 0);

  // // 4. JEC (Jet Energy Correction) 세부 정보 (이미 보정된 Pt/E를 쓰거나 Shift 값을 씀)
  // // GetAllJets()에서 이 값들을 Jet 객체에 넣는 코드가 없습니다.
  // fChain->SetBranchStatus("jet_JECL1FastJet", 0);
  // fChain->SetBranchStatus("jet_JECFull", 0);
  ...
  cout << "[Vcb_DL::initializeAnalyzer] RunNoJESBreakdown = " << run_no_jes_breakdown << endl;
...
void Vcb_DL::executeEventFromParameter(AnalyzerParameter param)
{
  ...
  // Jet selection
  vec_sel_jet = SelectJets(vec_this_jet, param.Jet_ID, JET_PT_DL, jet_eta_cut);
  vec_sel_jet = SelectJets(vec_sel_jet, param.PUJet_Veto_ID, JET_PT_DL, jet_eta_cut);
  ...
  //   if (IsDATA)
  //   {
  //     if (DataStream == "EGamma" || DataStream == "SingleElectron")
  //     else
  ...
  if (!IsDATA)
  {
    weight_sl_trig = mcCorr->SingleLepton_Trigger_SF("POGTight", mu_trig, vec_sel_muon, 0, "passMVAID_iso_WP80", el_trig, vec_sel_electron, 0);
    if (param.syst_ == AnalyzerParameter::Central && run_syst)
    {
      weight_sl_trig_el_down = mcCorr->SingleLepton_Trigger_SF("POGTight", mu_trig, vec_sel_muon, 0, "passMVAID_iso_WP80", el_trig, vec_sel_electron, -1);
      weight_sl_trig_el_up = mcCorr->SingleLepton_Trigger_SF("POGTight", mu_trig, vec_sel_muon, 0, "passMVAID_iso_WP80", el_trig, vec_sel_electron, 1);
      weight_sl_trig_mu_down = mcCorr->SingleLepton_Trigger_SF("POGTight", mu_trig, vec_sel_muon, -1, "passMVAID_iso_WP80", el_trig, vec_sel_electron, 0);
      weight_sl_trig_mu_up = mcCorr->SingleLepton_Trigger_SF("POGTight", mu_trig, vec_sel_muon, 1, "passMVAID_iso_WP80", el_trig, vec_sel_electron, 0);
    }
    ...
    {
      weight_mu_id *= mcCorr->MuonID_SF_Json(param.Muon_ID_SF_Key, vec_sel_muon[i].Eta(), vec_sel_muon[i].MiniAODPt(), 0);
      if (param.syst_ == AnalyzerParameter::Central && run_syst)
      {
        weight_mu_id_down *= mcCorr->MuonID_SF_Json(param.Muon_ID_SF_Key, vec_sel_muon[i].Eta(), vec_sel_muon[i].MiniAODPt(), -1);
        weight_mu_id_up *= mcCorr->MuonID_SF_Json(param.Muon_ID_SF_Key, vec_sel_muon[i].Eta(), vec_sel_muon[i].MiniAODPt(), 1);
      }

      weight_mu_iso *= mcCorr->MuonISO_SF_Json(param.Muon_ISO_SF_Key, vec_sel_muon[i].Eta(), vec_sel_muon[i].MiniAODPt(), 0);
      if (param.syst_ == AnalyzerParameter::Central && run_syst)
      {
        weight_mu_iso_down *= mcCorr->MuonISO_SF_Json(param.Muon_ISO_SF_Key, vec_sel_muon[i].Eta(), vec_sel_muon[i].MiniAODPt(), -1);
        weight_mu_iso_up *= mcCorr->MuonISO_SF_Json(param.Muon_ISO_SF_Key, vec_sel_muon[i].Eta(), vec_sel_muon[i].MiniAODPt(), 1);
      }
    }
    weight *= weight_mu_id;
  ...
  if (run_mm)
  {
    ...
    // if (vec_sel_muon.size() != 2 || vec_sel_electron.size() != 0)
      return;
  ...
  else if (run_me)
  {
    ...
    // if (vec_sel_muon.size() != 1 || vec_sel_electron.size() != 1)
      return;
  ...
  else if (run_ee)
  {
    ...
    // if (vec_sel_muon.size() != 0 || vec_sel_electron.size() != 2)
      return;
  ...
  if ((run_mm || run_ee) && abs(dilepton_mass - Z_MASS) < 15)
    return;
  ...
  if (!IsDATA)
  {
    ...
    if (param.syst_ == AnalyzerParameter::Central)
    {
      // float weight_c_tag_root = mcCorr->GetCTaggingReweight_1d(vec_sel_jet, vec_jet_tagging_para.at(1), "central");
...
float Vcb_DL::Get_Pt_Gen_TT(const vector<Gen> &vec_gen)
{
  int index_last_t = -999;
  int index_last_at = -999;
  for (unsigned int i = 0; i < vec_gen.size(); i++)
...
void Vcb_DL::Make_Result_Tree(AnalyzerParameter &param)
{
  ...
  bvsc_third_pt = vec_sel_jet[2].Pt();
  bvsc_fourth_pt = vec_sel_jet[3].Pt();

  bvsc_third_eta = vec_sel_jet[2].Eta();
  bvsc_fourth_eta = vec_sel_jet[3].Eta();

  pt_gen_tt = Get_Pt_Gen_TT(vec_gen);
...
void Vcb_DL::Set_Result_Tree()
{
  ...
  for (unsigned int i = 0; i < vec_channel.size(); i++)
  {
    for (unsigned int j = 0; j < vec_syst_type.size(); j++)
    {
      ...
      result_tree->Branch("n_c_jet", &n_c_jet);
      ...
      result_tree->Branch("bvsc_third_pt", &bvsc_third_pt);
      result_tree->Branch("bvsc_fourth_pt", &bvsc_fourth_pt);

      result_tree->Branch("bvsc_third_eta", &bvsc_third_eta);
      result_tree->Branch("bvsc_fourth_eta", &bvsc_fourth_eta);
